...
def standard_error_proportion(se=None, proportion=None, n=None):
    if se is None and proportion is not None and n is not None:
        se_answer=math.sqrt((proportion*(1-proportion))/(math.sqrt(n)))
        ...
        print_latex(f"$SE=\\sqrt{{\\frac{{ {proportion}(1-{proportion}) }}{{ {n} }} }} \\approx {se_answer}$ ")
        return se_answer
    elif proportion is None and n is not None and se is not None:
        proportion_answer_1=(1+math.sqrt(1-4*(math.sqrt(n)*(se**2))))/(2)
        proportion_answer_2=(1-math.sqrt(1-4*(math.sqrt(n)*(se**2))))/(2)
        ...
        print("Substituting the values we get:")
        print_latex(f"$p = \\frac{{1 \\pm \\sqrt{{1-4\\left({se}^2 \\cdot \\sqrt{n}\\right)}}}}{{2}} \\approx {proportion_answer_1} $ or ${proportion_answer_2} $")
        return proportion_answer_1, proportion_answer_2
    elif n is None and se is not None and proportion is not None:
        n_answer=((proportion-proportion**2)/(se**2))**2
        ...
        print("Substituting the values we get:")
        print_latex(f"$ n = \\left(\\frac{{{proportion}-{proportion}^2}}{{{se}^2}}\\right)^2 \\approx {n_answer}$")
        return n_answer
    else:
...
def test_clt_proportion(n,p):
    normal=input("Is the sampled population normal? (y/n)")
    if normal=='y':
        print_latex(f"Since the sampled population is normally distributed, the sampling distribution of $\\hat P \\approx N({p},{standard_error_proportion(n,p)})$, no matter what sample size you choose")
    else:
        symmetric = input("Is the sampled population approximately symmetric? (y/n)")
        if symmetric=='y':
            print_latex(f"Since the sampled population is approximately symmetric, the sampling distribution of $\\hat P \\approx N({p},{standard_error_proportion(n,p)})$, for relatively small values of $n$")
        else:
            ...
            if n*p>5 and n*(1 - p) > 5:
                print_latex(f"In this case since $np = {n*p :.2f} > 5$ and $n(1 - p) = {n*(1-p) :.2f} > 5$, the sampling distribution of $\\hat P \\approx N({p},{standard_error_proportion(p,n)})$")
            else:
                print_latex(f"In this case since $np = {n*p :.2f} \\le 5$ or $n(1-p) = {n*(1-p) :.2f} \\le 5$, the sampling distribution of  $\\hat P \\not\\approx N({p},{standard_error_proportion(p,n)})$")
